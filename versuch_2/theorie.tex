...
\end{equation}
mit $c$ als massebezogene Wärmekapazität, $m$ als Masse, $\rho$ als Dichte. 

Da $ \vec{j} $ als eine stetige Funktion angenommen werden kann, folgt aus dem Mittelwertsatz der Integralrechnung, dass ein $ \Theta \in [t; t + \Delta t] existiert mit:

\begin{equation}
\int_{t}^{t + \Delta t} dt' \int_{V} -\nabla \vec{j} dV = \Delta t \cdot \int_{V} -\nabla \vec{j(\Theta)} dV
\end{equation}. 

Betrachtet man nun $ \Delta t \to 0 $ mit $\Delta t \to \partial t $ und $\Delta T \to \partial T $, so ergibt sich:
\begin{equation}
\partial t \cdot \int_{V} -\nabla \vec{j(\Theta)} dV = \int_{V} \rho dV \cdot c \cdot \partial T
\end{equation}. 
 
Für den Grenzübergang gilt dann $\Theta \to t$ und entsprechend in differenzieller Form unter Berücksichtigung von \ref{form:j}: 
\begin{equation}
\rho \cdot c \cdot \partial T = -\nabla \vec{j(t)} \partial t = \kappa \cdot \triangle \vec{j(t)} \partial t
\end{equation}
